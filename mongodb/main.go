...
func main() {
	...
	// default if using 'make up' to spin up MongoDB locally
	dsn := "mongodb://localhost:27017/estoria?replicaSet=rs0"
	...
	// the default strategy uses a single collection for all events
	strategy, err := mongostrategy.NewSingleCollectionStrategy(client, client.Database("estoria").Collection("events"))
	check(err)
